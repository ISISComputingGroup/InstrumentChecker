...
class ChannelAccessUtils(object):
    ...
    def get_value(self, pv: str, timeout: float = None) -> PVValue:
        """
    ...
    @staticmethod
    def _dehex_and_decompress(data: str) -> bytes:
        """
        Converts the raw data from a PV to a decompressed string.
        :param data: The raw data from the PV. It is a string of numbers
        representing the bytes of the raw data of the PV.
        :return: The data of the PV in the form of a decompressed and decoded string.
        """
        return zlib.decompress(binascii.unhexlify(data))

    def get_inst_list(self) -> list:
        """
    ...
    def get_interesting_pvs(self) -> set:
        """
        Returns all PVs with high or medium interest status from the corresponding
        instrument PV. The instrument for which it returns the list depends on the
        prefix assigned to this class, which needs to be in the format IN:NAME_OF_INSTRUMENT.
    ...
    def _get_pvs_by_interesting_level(self, interesting_level: PvInterestingLevel) -> list:
        """
        Returns the list of all PVs with the specified interesting level from the
        corresponding instrument PV. The instrument for which it returns the list
        depends on the prefix assigned to this class, which needs to be in the
    ...
    def get_protected_iocs(self) -> list:
        """
        Gets the names of all protected IOCS from the PV of IOCs of the instrument.
        Protected IOCs are IOCs that a user is not allowed to stop.
        :return: a list of strings representing IOC names.
        """
